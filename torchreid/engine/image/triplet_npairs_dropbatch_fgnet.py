...
class ImageTripletNpairsDropBatchFGnetEngine(engine.Engine):
    ...
    def __init__(self, datamanager, model, optimizer, margin=0.3, margin_sasc=0.3, margin_sadc=0.2, margin_dasc=0.4,
                 weight_t=1, weight_x=1, weight_n=1, weight_db_t=1, weight_db_x=1, scheduler=None, use_gpu=True,
                 label_smooth=True):
        super(ImageTripletNpairsDropBatchFGnetEngine, self).__init__(datamanager, model, optimizer, scheduler, use_gpu)
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            outputs, features, parts, db_logits, db_features = self.model(imgs)
            ...
            loss_db_x = self._compute_loss(self.criterion_db_x, db_logits, pids)
            loss_db_t = self._compute_loss(self.criterion_db_t, db_features, pids)
            loss = self.weight_t * loss_t + self.weight_x * loss_x + self.weight_n * loss_n + self.weight_db_t * loss_db_t + self.weight_db_x * loss_db_x
            loss.backward()
            self.optimizer.step()
