...
from torchreid.losses import CrossEntropyLoss, TripletLoss, GroupLoss
from torchreid.utils import AverageMeter, open_specified_layers, open_all_layers
from torchreid import metrics


class ImageGroupTripletDropBatchDropBotFeaturesEngine(engine.Engine):
    ...
    def __init__(self, datamanager, model, optimizer, margin=0.3,
                 weight_t=1, weight_g=1, weight_x=1, weight_db_t=1, weight_db_g=1, weight_db_x=1, weight_b_db_t=1, weight_b_db_g=1, weight_b_db_x=1, scheduler=None, use_gpu=True,
                 label_smooth=True, top_drop_epoch=-1):
        super(ImageGroupTripletDropBatchDropBotFeaturesEngine, self).__init__(datamanager, model, optimizer, scheduler, use_gpu)

        self.weight_t = weight_t
        self.weight_g = weight_g
        self.weight_x = weight_x
        self.weight_db_t = weight_db_t
        self.weight_db_g = weight_db_g
        self.weight_db_x = weight_db_x
        self.weight_b_db_t = weight_b_db_t
        self.weight_b_db_g = weight_b_db_g
        self.weight_b_db_x = weight_b_db_x
        self.top_drop_epoch = top_drop_epoch

        self.criterion_g = GroupLoss()
        ...
        self.criterion_b_db_g = GroupLoss()
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            loss_g = self._compute_loss(self.criterion_g, features, pids)
            ...
            loss_db_g = self._compute_loss(self.criterion_db_g, db_features, pids)
            ...
            loss_b_db_g = self._compute_loss(self.criterion_b_db_g, b_db_features, pids)
            loss = self.weight_t * loss_t + self.weight_g * loss_g + self.weight_x * loss_x + self.weight_db_t * loss_db_t + self.weight_db_g * loss_db_g + self.weight_db_x * loss_db_x + self.weight_b_db_t * loss_b_db_t + self.weight_b_db_g * loss_b_db_g + self.weight_b_db_x * loss_b_db_x
            loss.backward()
            self.optimizer.step()
            ...
            if (batch_idx+1) % print_freq == 0:
                ...
                print('Epoch: [{0}/{1}][{2}/{3}]\t'
                      ...
                      'Loss_db_g {loss_db_g.val:.4f} ({loss_db_g.avg:.4f})\t'
                      'Loss_db_x {loss_db_x.val:.4f} ({loss_db_x.avg:.4f})\t'
                      'Loss_b_db_t {loss_b_db_t.val:.4f} ({loss_b_db_t.avg:.4f})\t'
                      'Loss_b_db_g {loss_b_db_g.val:.4f} ({loss_b_db_g.avg:.4f})\t'
                      ...
                      loss_db_g=losses_db_g,
                      loss_db_x=losses_db_x,
                      loss_b_db_t=losses_b_db_t,
                      loss_b_db_g=losses_b_db_g,
            ...
            if self.writer is not None:
                ...
                self.writer.add_scalar('Train/Loss_g', losses_g.avg, n_iter)
                self.writer.add_scalar('Train/Loss_x', losses_x.avg, n_iter)
                self.writer.add_scalar('Train/Loss_db_t', losses_db_t.avg, n_iter)
                self.writer.add_scalar('Train/Loss_db_g', losses_db_g.avg, n_iter)
                ...
                self.writer.add_scalar('Train/Loss_b_db_g', losses_b_db_g.avg, n_iter)
