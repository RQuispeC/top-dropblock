...
from torchreid.losses import CrossEntropyLoss, TripletLoss, NPairsLoss, SeparationLoss
from torchreid.utils import AverageMeter, open_specified_layers, open_all_layers
from torchreid import metrics


class ImageTripletNpairsSoftmaxSeparateFGnetEngine(engine.Engine):
    ...
    def __init__(self, datamanager, model, optimizer, margin=0.3, margin_sasc=0.3, margin_sadc=0.2, margin_dasc=0.4,
                 weight_t=1, weight_x=1, weight_n=1, weight_x_parts=1, weight_separate = 1, scheduler=None, use_gpu=True,
                 label_smooth=True):
        super(ImageTripletNpairsSoftmaxSeparateFGnetEngine, self).__init__(datamanager, model, optimizer, scheduler, use_gpu)
        ...
        self.criterion_separate = SeparationLoss(
            use_gpu=self.use_gpu
        )
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            weighted_tensors = [x.unsqueeze(0) for (x, _) in part_weights]
            #print(len(weighted_tensors), weighted_tensors[0].size(), torch.cat(weighted_tensors).size())
            weighted_tensors = torch.cat(weighted_tensors).permute(1, 0, 2, 3, 4)
            loss_separate = self._compute_loss(self.criterion_separate, weighted_tensors, pids)
            loss = self.weight_t * loss_t + self.weight_x * loss_x + self.weight_n * loss_n + self.weight_x_parts*loss_x_parts + self.weight_separate * loss_separate
            ...
            losses_separate.update(loss_separate.item(), pids.size(0))
            ...
            if (batch_idx+1) % print_freq == 0:
                ...
                print('Epoch: [{0}/{1}][{2}/{3}]\t'
                      ...
                      'Loss_separate {loss_separate.val:.4f} ({loss_separate.avg:.4f})\t'
            ...
            if self.writer is not None:
                ...
                self.writer.add_scalar('Train/Loss_separate', losses_separate.avg, n_iter)
