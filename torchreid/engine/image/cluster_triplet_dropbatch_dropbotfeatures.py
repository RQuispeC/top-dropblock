...
from torchreid.losses import CrossEntropyLoss, TripletLoss, ClusterLoss
from torchreid.utils import AverageMeter, open_specified_layers, open_all_layers
from torchreid import metrics


class ImageClusterTripletDropBatchDropBotFeaturesEngine(engine.Engine):
    ...
    def __init__(self, datamanager, model, optimizer, margin=0.3,
                 weight_t=1, weight_x=1, weight_c=1, weight_db_t=1, weight_db_x=1, weight_db_c=1,weight_b_db_t=1, weight_b_db_x=1, weight_b_db_c=1, scheduler=None, use_gpu=True,
                 label_smooth=True, top_drop_epoch=-1):
        super(ImageClusterTripletDropBatchDropBotFeaturesEngine, self).__init__(datamanager, model, optimizer, scheduler, use_gpu)
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            loss = self.weight_t * loss_t + self.weight_x * loss_x + self.weight_db_t * loss_db_t + self.weight_db_x * loss_db_x + self.weight_b_db_t * loss_b_db_t + self.weight_b_db_x * loss_b_db_x + self.weight_c*loss_c + self.weight_db_c*loss_db_c + self.weight_b_db_c*loss_b_db_c
            loss.backward()
            self.optimizer.step()
