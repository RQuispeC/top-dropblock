...
class ImageTripletNpairsSoftmaxCsattentionFGnetEngine(engine.Engine):
    ...
    def __init__(self, datamanager, model, optimizer, margin=0.3, margin_sasc=0.3, margin_sadc=0.2, margin_dasc=0.4,
                 ...
                 label_smooth=True):
        super(ImageTripletNpairsSoftmaxCsattentionFGnetEngine, self).__init__(datamanager, model, optimizer, scheduler, use_gpu)
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            prelogits_global, features_global, prelogits_parts, features_parts, attention_maps = self.model(imgs)
            loss_t = self._compute_loss(self.criterion_t, features_global, pids)
            loss_x = self._compute_loss(self.criterion_x, prelogits_global, pids)
            loss_x_parts = self._compute_loss(self.criterion_x_parts, prelogits_parts, pids)
            loss_n = self._compute_loss(self.criterion_n, features_parts, pids)
            ...
            accs.update(metrics.accuracy(prelogits_global, pids)[0].item())
            ...
            if self.writer is not None:
                ...
                #log attention
                for i, (_, channel_weights, spatial_attention, tensor_attention) in enumerate(attention_maps):#parts
                    for j in range(min(tensor_attention.size(0), 3)):#batch
                        self.writer.add_histogram('Train/Part{}/Batch{}/channel_attention'.format(i, j), channel_weights[j, ...], n_iter)
                        self.writer.add_histogram('Train/Part{}/Batch{}/tensor_attention'.format(i, j), tensor_attention[j, ...], n_iter)
                        if epoch % 5 == 0:
                            self.writer.add_image('Train/Part{}/Batch{}/spatial_attention'.format(i, j), spatial_attention[j, ...], n_iter, dataformats='CHW')
